**更新対象ファイル:**  
`md_/現在の機能説明一覧_マークダウン_11121_v1.md`

以下が **最新状態（2025/11/22 時点）** と、**現在のリポジトリにあるコード・構成と整合させた内容** です。  
手動でファイルを書き換える際は、この全文をそのまま貼り付けてください。
...
│   ├─ moving_target.py    # **斜め移動保証ロジック**（位置更新と反射処理）
│   └─ moving_target_manager.py  # **速度レベル（1?5）設定機能** とターゲット管理
│
...
│       - 3102a320-1eac-493e-8e8c-bc8aafaceff7.png   # 新規追加画像例
│
...
- **ポイント**  
  ...
  - 速度が 0 の場合は再ランダム化して必ず斜め移動になるよう保証。
...
class MovingTargetManager:
    ...
    def add_target(self, image_path: str,
        ...
        if initial_position is None:
            ...
            x = random.randint(xmin, xmax - 100)   # 画像幅分余白確保
...
- コンボボックスで **速度レベル (1?5)** を選択可能。`register_target()` が `speed_level = self.speed_combo.currentIndex() + 1` と取得し、`MainWindow.show_register_moving_target()` に渡す設計。
...
- 画像読み込みは絶対パスで行い、`cv2.imread()` が失敗した場合は赤枠の矩形描画にフォールバック。  
- タイマー駆動でフレーム更新 → `MovingTarget.update()` による位置・速度計算 → QPainter で描画。
...
`backend/moving_target.py` の `update()` が **dx または dy が 0** のときに再ランダム化し、必ず斜め方向へ移動させます。

### 画像形式の推奨

- PNG は libpng の ICC プロファイル警告が出るため **JPEG に変換して使用**（例: `assets/targets/3102a320-1eac-493e-8e8c-bc8aafaceff7.png` → `3102a320-1eac-493e-8e8c-bc8aafaceff7.jpg`）。  
- JPEG であれば OpenCV の警告は出ません。
...
以上が最新の機能説明マークダウンです。ご自身でファイルを書き換えていただく際にご活用ください。