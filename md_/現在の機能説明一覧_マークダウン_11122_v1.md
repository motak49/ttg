...
以下が **最新状態（2025/11/22 時点）** と、**本チャットで行ったコード修正を反映した内容** です。手動でファイルを書き換える際は、この全文をそのまま貼り付けて保存してください。
...
- **`_show_collision_stop_message()`**  
  - トラッキングを一時停止し、モーダル `QMessageBox` を表示。OK 押下後に 1?秒待機してから `resume_tracking()` を呼び出すことで、ユーザーが結果を見る時間を確保します。
...
- **デバッグログ出力の絞り込み**  
  - 角度・距離のデバッグ情報は、衝突が実際に判定された (`angle_deg > ANGLE_THRESHOLD and dist_to_edge <= DIST_TOLERANCE`) ときのみ `print("[DEBUG] angle_deg=..., dist_to_edge=...")` が出力されます。これにより不要なログが削減され、デバッグ時の可読性が向上しました。
...
1. **対象ファイルを開く**: `md_/現在の機能説明一覧_マークダウン_11121_v4.md`  
