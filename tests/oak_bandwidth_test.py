...
# depthai 3.1.0 では XLinkIn/Out が廃止されたためスキップ
@pytest.mark.skip(reason="XLinkIn/Out removed in depthai 3.1.0")
def test_oak_bandwidth():
    N = 50
    ...
    with dai.Device(pipeline, maxUsbSpeed=dai.UsbSpeed.SUPER_PLUS) as device:
        ...
        for i in range(N):
            ...
            if i == DISCARD_N-1:
                ...
                pass
        # print(f'{N}th buffer received at {time.time()}')    total_time = receive_ts[-1] - receive_ts[DISCARD_N-1]
    total_bits = (N-DISCARD_N) * SIZE * 8
    downlink = total_bits / total_time
