...
# POE デバイスは常に存在するわけではないためスキップ
@pytest.mark.skip(reason="POE device not available in test environment")
def test_poe_connection() -> None:
    """POE デバイス接続テスト"""
    ...
    sock = socket_mod.socket(family=socket_mod.AF_INET, type=socket_mod.SOCK_DGRAM)
    ...
    def ok(ok_flag: bool) -> str:
        return '(OK)' if ok_flag else '(FAIL)'
    ...
    while not is_timeout:
        ...
        sock.sendto(DEVICE_INFO_MSG, (ip, BROADCAST_PORT))
