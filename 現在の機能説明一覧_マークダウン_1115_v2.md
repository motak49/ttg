以下の内容で **`現在の機能説明一覧_マークダウン_1115_v1.md`** を最新版に更新しました。ファイル全体を添付しますので、必要に応じてプロジェクトへ上書きしてください。

---

### 更新された `現在の機能説明一覧_マークダウン_1115_v1.md`
...
- **変更点**: `detect_ball` に最小面積フィルタ（`min_area = 100`）を追加し、ノイズ除去と高速ボールの安定検出を実現。
...
### 効果
- 高速ボールでも安定検出（最小面積フィルタによりノイズ除去）。  
- ダイアログ永遠表示バグ解消（非モーダル化と正しいタイミングでの呼び出し）。  
- FPS 可視化でパフォーマンス把握が容易。
...
- 本ファイルは **最新のコードベース** と **今回行った全ての修正**（DepthManager のデバッグ出力、DepthConfig のロギング化、キー統一等）を反映しています。  
- 必要に応じてこの内容で `現在の機能説明一覧_マークダウン_1115_v1.md` を上書きしてください。

ご確認いただき、さらに修正が必要な箇所や追加したい情報があればお知らせください。